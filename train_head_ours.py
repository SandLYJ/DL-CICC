...
def infer(model, dataloader):
    ...
    with torch.no_grad():
        for iter, (image) in enumerate(dataloader):
            image = image[0].cuda()
            _, logit_image, _, _ = model(image, image,None)
...
def add_gaussian_noise(features, noise_level=0.1):
    noise = torch.randn_like(features) * noise_level
    return features + noise

if __name__ == "__main__":
    dataset = "STL-10"  # ["CIFAR-10", "CIFAR-20", "STL-10", "ImageNet-10", "ImageNet-Dogs", "DTD", "UCF101", "ImageNet"]
    ...
    # labels_test = np.loadtxt("./data/" + dataset + "_labels_train.txt")
    ...
    # dataloader_test = DataLoader(
    #     dataset_image_train, batch_size=batch_size, shuffle=False, drop_last=False
    # )
    ...
    instance_loss = InstanceLoss(batch_size, temperature=instance_temperature)
    cluster_loss = ClusterLoss(cluster_num, temperature=cluster_temperature)
    # hsd_loss = HSD(tau=0.05)
    print("Start training...")
    preds, _ = infer(model, dataloader_test)
    cluster_metric(labels_test, preds)
    for epoch in range(epochs):
        start = time.time()
        model.train()
        loss_distill_epoch = loss_instance_epoch = loss_entropy_epoch = 0
        for iter, (text, image, neigh_text, neigh_image) in enumerate(dataloader_train):
            text = text[0].cuda()
            image = image[0].cuda()

            image_add = add_gaussian_noise(image).cuda()
            ...
            logit_text, logit_image,logit_image_add, logit_text_ins, logit_image_ins,logit_image_ins_add = model(text, image,image_add)
            neigh_logit_text, neigh_logit_image,neigh_logit_text_ins, neigh_logit_image_ins = model(neigh_text,
                                                                                                     neigh_image,None)

            loss_distill = distill_loss(logit_image, neigh_logit_text) + distill_loss(
                logit_text, neigh_logit_image
            )
            # loss_distill = distill_loss(logit_image, logit_text)

            loss_consist = consistency_loss(logit_text, logit_image)
            loss_entropy = entropy(logit_text) + entropy(logit_image)

            # logit_text_ins = logit_text_ins + torch.randn_like(logit_text_ins) * 0.001
            # logit_image_ins = logit_image_ins + torch.randn_like(logit_image_ins) * 0.001

            # loss_instance = instance_loss(logit_text_ins, logit_image_ins)

            loss_instance_add = instance_loss(logit_image_ins,logit_image_ins_add)
            loss_cluster_add = cluster_loss(logit_image,logit_image_add)

            loss_instance = instance_loss(logit_image_ins, neigh_logit_text_ins)+instance_loss(logit_text_ins, neigh_logit_image_ins)
            if epoch < 20:
                loss = loss_distill + loss_consist - 5 * loss_entropy
            else:
                loss = loss_distill + loss_instance + 0.01 *loss_instance_add +0.01 *loss_cluster_add - 5 * loss_entropy
            ...
            if (iter + 1) % 50 == 0 or iter + 1 == len(dataloader_train):
                print(
                    "[Epoch {}/{}] [Iter {}/{}] Loss Cluster: {:.4f} Loss Instance: {:.4f} Loss Entropy: {:.4f}".format(
                        epoch + 1,
                        epochs,
                        iter + 1,
        ...
        print(
            "[Epoch: {}] Loss Cluster: {:.4f} Loss Instance: {:.4f} Loss Entropy: {:.4f}".format(
                epoch + 1,
                loss_distill_epoch / (iter + 1),
                loss_instance_epoch / (iter + 1),
        ...
        cluster_metric(labels_test, preds)
        # print("Instance Head Result:")
        # preds = inference(model, dataloader_test, cluster_num)
        # cluster_metric(labels_test, preds)

        # for p in [60]:
        #     tsne(confidences_image, preds, epoch, p)
