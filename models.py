...
class Network(nn.Module):
    ...
    def forward(self, text, image,image_add):
        if image_add is not None:
            logit_text = self.cluster_head_text(text)
            logit_image = self.cluster_head_image(image)
            logit_image_add = self.cluster_head_image(image_add)
            ...
            logit_image_ins_add = self.instance_head_image(image_add)
            return logit_text, logit_image,logit_image_add, logit_text_ins, logit_image_ins ,logit_image_ins_add
        else:
            logit_text = self.cluster_head_text(text)
            logit_image = self.cluster_head_image(image)
