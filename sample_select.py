...
import torch
import numpy as np
from  save import save_pl_samples
...
def inference(loader, model, device):
    ...
    for step, (x, y, index) in enumerate(loader):
        ...
        embedding_vector.extend(embedding.cpu().detach().numpy())          # after ins_projector
        embedding_512_vector.extend(embedding_512.cpu().detach().numpy())  # only backbone
        prediction_vector.extend(c.cpu().detach().numpy())                 # after clu_projector   硬聚类结果
        confidence_vector.extend(c2.cpu().detach().numpy())                # after clu_projector   soft聚类结果
        labels_vector.extend(y.numpy())                                    # label
        if step % 20 == 0:
...
if __name__ == "__main__":
    ...
    model_state_dict = torch.load(model_fp, map_location=device.type)['net']  # bianchengle yibu ,zhijie quchu wangluo de kv qitade youhuaqi epoch meiqu
    ...
    #data_x     after ins_projector  only backbone  硬聚类结果  soft聚类结果  label
    print("Y.shape: ", Y.shape)
    print(np.unique(Y))
    print("X1.shape: ", X1.shape)
    print(np.unique(X1))
    nmi, ari, f, acc = evaluation.evaluate(Y, X1)
    print('NMI = {:.4f} ARI = {:.4f} F = {:.4f} ACC = {:.4f}'.format(nmi, ari, f, acc)) # IN-10 NMI = 0.8746 ARI = 0.8372 F = 0.8535 ACC = 0.8951
    ...
    for i in range(class_num):
        distances[:, i] = np.dot(embedding_512, centers[i]) / np.linalg.norm(centers[i]) # 样本到类中心的余弦相似性
    print(f"distances.shape: {distances.shape}")
    cluster_center = np.argmax(distances, axis=0) # 获取每类距离最大的样本索引
    print(f"cluster_center: {cluster_center}")

    for i in range(class_num):
        image_center[i] = np.transpose(origin[cluster_center[i]], (1, 2, 0))

    nearing_top1 = np.argsort(-distances, axis=1)[:, 0].reshape(-1, 1)    # 每个样本最接近的类别索引
    nearing_top2 = np.argsort(-distances, axis=1)[:, :2]                  # 每个样本最接近的两个类别索引
    nearing_top = np.argsort(-distances, axis=1)                          # 完整的接近类别排序

    hardness = torch.zeros(Y.shape[0])
    for i in range(Y.shape[0]):
        hardness[i] = distances[i, nearing_top2[i, -2]] - distances[i, nearing_top2[i, -1]]
        # 反映样本在最相似类与次相似类之间的“模糊程度”。值越接近 0，说明样本在两个类之间较难区分（更“困难”）。
    hardness = 1 - hardness
    ...
    matrix = construct_matrix(val_data_with_index, model, device) # 所有批次特征
    print(f"matrix.shape: {matrix.shape}")

    selection_result = selection(matrix.cpu(), hardness, budget=args.budget, beta=1., gamma=1.) # diversity and representiveness
    ...
    pos_distribution_gt_origin = np.zeros(class_num)
    pos_distribution_pre = np.zeros(class_num)
    neg_distribution_gt = np.zeros(class_num)
    neg_distribution_gt_origin = np.zeros(class_num)
    neg_distribution_pre = np.zeros(class_num)

    # 直接根据匈牙利匹配对齐gt和聚类结果
    for i in selection_result:   # sample 12697 nearest five  center (9,0,2,1,8)
        if label_adjusted[i] in nearing_top[i, :args.candidate_num]:  # candidate_num=5   检查该样本的调整标签（label_adjusted[i]）是否在其5最近的候选标签中
            pos_index.append(i)                      #12697  11559
            pos_label.append(label_adjusted[i])      # 9      7
            for j in range(args.candidate_num):
                if nearing_top[i, j] != label_adjusted[i]:
                    pos_candidate.append(nearing_top[i, j]) # 将所有不等于该标签的候选标签添加到 pos_candidate 中 ((1)0,2,1,8, (2)5,3,8,4)
            #更新正样本的标签分布
            pos_distribution_gt[label_adjusted[i]] += 1
            pos_distribution_gt_origin[Y[i]] += 1
            pos_distribution_pre[X1[i]] += 1

            selected_index.append(i)
            selected_label.append(label_adjusted[i])
        else:
            neg_index.append(i) # 6543,  8432, 10791,  2865,  8361, 11997, 12764, 12172
            for j in range(args.candidate_num): # all push into neg_
                neg_label.append(nearing_top[i, j])
            neg_distribution_gt[label_adjusted[i]] += 1
            neg_distribution_gt_origin[Y[i]] += 1
            neg_distribution_pre[X1[i]] += 1
            print(f"neg_distribution_gt: {label_adjusted[i]}") # 4  6  7 0  6  9 9  9
            print(f"neg_distribution_pre: {X1[i]}")            # 9  2  9 2 9  2  8  2



            selected_index.append(i)  # 12697
            selected_label.append(-1)   # 9
    ...
    print(f"pos_distribution_gt: {pos_distribution_gt}")       # label_adjusted  #pos_distribution_gt: [37. 40. 40. 44. 51. 52. 53. 68. 69. 38.]
    # print(f"pos_distribution_gt_origin: {pos_distribution_gt_origin}") Y pos_distribution_gt_origin: [40. 40. 37. 52. 69. 51. 53. 44. 68. 38.]
    print(f"pos_distribution_pre: {pos_distribution_pre}")
    print(f"neg_distribution_gt: {neg_distribution_gt}")                # [1. 0. 0. 0. 1. 0. 2. 1. 0. 3.]
    # print(f"neg_distribution_gt_origin: {neg_distribution_gt_origin}")  [0. 0. 1. 0. 0. 1. 2. 0. 1. 3.]
